...
======================================================================
Loads a checkpoint produced by sft_train_v2.py and generates responses
to instruction prompts using the exact Alpaca prompt format that
sft_prepare_v2.py used during data preparation.

Prompt format (mirrors sft_prepare_v2.py exactly)
-----------------------------------------------
  Without context:
      ### SYSTEM:
...
--------------
  ...
  # Eval file mode — run model on held-out examples from sft_prepare_v2.py
  python sft_generate.py ./sft_checkpoints/sft_ckpt_final.pt \\
...
def build_inference_prompt(system_prompt: str, instruction: str, context: str = "") -> str:
    """
    Build the Alpaca-format prompt string that matches what sft_prepare_v2.py
    ...
    return build_sft_prompt(system_prompt, instruction, context)
...
def generate_tokens(
    ...
    """
    recent_tokens = []   # optional sliding window for legacy stop sequences
...
def run_single(
    ...
    """
    prompt       = build_inference_prompt(system_prompt, instruction, context)
...
def run_eval_file(
    ...
    """
    Load val_examples.jsonl produced by sft_prepare_v2.py and run the model on
    ...
    if not os.path.exists(eval_file):
        ...
        print("  Run sft_prepare_v2.py first to generate val_examples.jsonl.")
        return
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "checkpoint",
        type=str,
        help="Path to the SFT checkpoint (.pt) produced by sft_train_v2.py.",
    )
    ...
    prompt_group.add_argument(
        ...
        metavar="PATH",
        help=(
            "Path to val_examples.jsonl produced by sft_prepare_v2.py. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Triggers the WITH_INPUT template (same as sft_prepare_v2.py). "
...
def main() -> None:
    ...
    print(f"    system_prompt      = {system_prompt[:80]}{'...' if len(system_prompt) > 80 else ''}")

    # ── Dispatch ──────────────────────────────────────────────────────────────
    if args.interactive:
        run_interactive(model, enc, block_size, system_prompt, args, device)
    elif args.eval_file:
        run_eval_file(
            model         = model,
            enc           = enc,
